...
export default function Assets() {
  ...
  const apiUrlSummary = "http://192.168.43.32:8000/api/finance/assests/summary/";
  ...
  return (
    ...
    >
          ...
          <View key={item.main_category}>
            <View className="flex-row justify-between bg-white p-4 rounded-2xl mb-3 shadow-lg">
              <Text className="text-[#013378] font-medium text-[16px]">
                {item.main_category} ({item.sub_item_count} items)
              </Text>
      ...
      <View className="flex-row justify-between mt-8">
        ...
        <TouchableOpacity
          onPress={() => router.push("/(Assets)/AssestsList")}
